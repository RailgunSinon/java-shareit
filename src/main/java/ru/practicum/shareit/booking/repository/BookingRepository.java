...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query(value = "select b from Booking b where b.item.userId = ?1 and b.status = ?2 "
        + "order by b.bookingStart desc ")
    List<Booking> findAllByOwnerIdAndState(long ownerId, Status status);
    ...
    @Query(value = "select b from Booking b where b.item.id = ?1 and ( b.bookingEnd < ?2 or b.bookingStart < ?2) "
